...
- [ ] 06-01-PLAN.md — Deterministic ops stage end-to-end: three ops tools + Stage 2–3 fixtures (creep/clean variants) + `OpsRunner` seam + typed `OpsResult`/`OpsSlice` cards + `POST /advance?stage=ops` (409 guard, `fixture` Literal, 503 fail-fast) — API-03, OPS-01..04, DEMO-01, SC2–SC5 offline
...
| 6. Ops Agent, Fixtures & Full Supervisor Wiring | 0/2 | Planned | - |
